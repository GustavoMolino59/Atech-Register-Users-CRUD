...
// Estado reativo para os dados da API
...
async function fetchData() {
    try {
        const response = await api.get(`/users/${id.value}`)
        ...
        response.data.tags.map((tag) => tags.value.push(tag.title))
        description.value = response.data.user.description
    } catch (error) {
...
async function handleUser() {
    try {
        ...
        if (!emailError.value && !nameError.value) {
            const response = await api.put(`/users/${id.value}`, {
                email: email.value,
                avatar:'',
                name: name.value,
                phone: phone.value,
...
<template>
    ...
    <main v-if="userData" class="container ">
        ...
        <div class="cointainer-fluid card mt-2 p-2">
            ...
            <form id="edit-form needs-validation" novalidate @submit.prevent="onSubmit">
                ...
                <div class="col-12 text-center mt-5">
                    <button class="btn btn-secondary btn-lg m-2" type="submit" @click="handleUser">Editar Usuário
                        Usuário</button>
                    <button class="btn btn-danger btn-lg" data-bs-toggle="modal" data-bs-target="#exampleModal">Deletar
                        o Usuário</button>
                </div>
            </form>

            <!--Pop-up certeza da deleção!-->
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <strong>Essa ação é permanente</strong>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-primary" data-bs-dismiss="modal">Cancelar</button>
                            <button type="button" class="btn btn-danger" data-bs-dismiss="modal" @click="deleteUser">Deletar</button>
                        </div>
                    </div>
                </div>
            </div>
